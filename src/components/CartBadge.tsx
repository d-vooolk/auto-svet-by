...
export function CartBadge({ currencySymbol }: { currencySymbol: string }) {
  ...
  return (
    <Link
      href="/cart/"
      className="group relative flex items-center gap-2.5 rounded-xl px-3 py-2 transition-colors hover:bg-brand-50"
      aria-label={count > 0 ? `Корзина, товаров: ${count}` : "Корзина пуста"}
    >
      <span className="relative">
        <CartIcon className="h-6 w-6 text-brand-600" />
        {count > 0 && (
      ...
      <span className="hidden text-left lg:block">
        <span className="block text-xs leading-tight text-brand-400">
          Корзина
        </span>
        <span className="tnum block text-sm leading-tight font-semibold text-brand-900">
